$.extend({
	...
	is: {
		...
		{
                        //icno = $.trim(icno);
                        //icno = icno.str_replace('-', '');
                        return ($.is.integer(icno) && icno.length == 12);
...
//				var flag = (jelm.hasClass('cm-on') || jelm.attr('checked'));
                                var flag = (jelm.hasClass('cm-on') || jelm.is(':checked'));

				// Get class
				var suffix = elm.className.match(/cm-check-items(-[^\s]+)?/i)[1] || '';

				$('input.cm-item' + suffix + '[type=checkbox]:not(:disabled)', elm.form).prop('checked', (flag == true) ? 'checked' : '');
	...
	this.check_fields = function()
	{
		...
		for (i=0; i < elms.length; i++) {
			...
			if (lbl.hasClass('cm-email')) {
				if ($.is.email(elm.val()) == false) {
					if (lbl.hasClass('cm-required') || $.is.blank(elm.val()) == false) {
                        ...
                        // Check the IC NO field
			if (lbl.hasClass('cm-icno')) {
				if ($.is.icno(elm.val()) == false) {
					if (lbl.hasClass('cm-required') || $.is.blank(elm.val()) == false) {
						this.form_message(lang.error_validator_icno, lbl);
						is_ok = false;
						set_mark = true;
					}
				}
			...
			// Check for blank value
				...
				// Check for multiple selectbox
				if (elm.is(':input')) {
					if (lbl.hasClass('cm-required') && ((elm.is(':checkbox') && !elm.is(':checked')) || $.is.blank(elm.val()) == true || elm.ceHint('is_hint'))) {
