...
(function($){
	...
	Zoomple.prototype.showZoom = function($img,e ){
		...
		this.delaier("/images/simulators/lens_back_1.png",$img.attr('alt'));
	}
	...
	Zoomple.prototype.delaiedZoom = function(imgRefUrl,imgDescription){
		...
		$(objImagePreloader).load(function() {	
			if(self.stopLoading){	
				...
				self.$holder.find("img").css({"width" : self.options.overlayWidth + "px","height" : self.options.overlayHeight + "px"}).attr({"src":src, "width" : this.width, "height" : this.height});
	...
	Zoomple.prototype.setImageOverlay = function(){
		...
		$overlay.css({'left': Math.round($img.offset().left), 'top': Math.round($img.offset().top), 'width': this.options.overlayWidth, 'height': this.options.overlayHeight});
	}
